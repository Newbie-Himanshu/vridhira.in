...
export default function LoginPage() {
  ...
  // Redirection & Sync Logic
  useEffect(() => {
    if (user && customer && !loading) {
      if (customer.isVerified) {
        // Essential: Sync visitor cart to cloud upon first verification/login
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    ...
    if (otp === '123456') {
      ...
      // Logic in useEffect will trigger sync and redirect
    } else {
      ...
      <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm">Access to the heart of Indian craftsmanship.</p>
          </div>
        </div>
        ...
        {error && (
          <Alert variant="destructive" className="rounded-2xl border-2 bg-destructive/5">
            ...
            <AlertDescription className="text-xs italic">{error.hint}</AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-6">
          <Button 
            variant="outline" 
            className="w-full h-14 rounded-2xl border-2 font-bold bg-white shadow-lg animate-pulse-glow" 
              ...
              <Card className="border-none shadow-2xl bg-white/70 backdrop-blur-md rounded-3xl">
                ...
                <CardContent className="space-y-4">
                  <form onSubmit={handleSignIn} className="space-y-4">
                    ...
                    <Button className="w-full h-12 rounded-2xl bg-primary text-white font-bold" disabled={loading}>
        ...
        <div className="text-[10px] text-muted-foreground px-8 text-center flex items-center justify-center gap-2">
          <ShieldCheck className="h-4 w-4" /> 
          Secure Heritage Access 
          <Separator orientation="vertical" className="h-3" />
          <Link href="/" className="hover:text-primary">Cancel & Return</Link>
        </div>
      </div>
    </div>
  );
