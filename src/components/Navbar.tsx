            ...
            <div className="lg:hidden pointer-events-auto">
              <Sheet onOpenChange={(open) => { if(!open) setIsMobileSearchActive(false); }}>
                ...
                <SheetContent side="right" className="w-full sm:max-w-md p-0 overflow-hidden border-none flex flex-col">
                  <SheetHeader className="px-10 pt-12 pb-8 text-left border-b bg-muted/30">
                    <SheetTitle className="font-headline text-4xl text-secondary flex items-center gap-5 group cursor-pointer transition-all duration-300">
                      <div className="relative w-12 h-12 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                          ...
                          <span className="relative text-white font-bold text-2xl">V</span>
                      </div>
                      <span className="transition-colors duration-300 group-hover:text-primary">Vridhira</span>
                    </SheetTitle>
                  </SheetHeader>
                  <div className="flex-1 px-10 py-12 space-y-14 overflow-y-auto">
                          ...
                          <div className="relative transition-all duration-500">
                            {isMobileSearchActive ? (
                              <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                                <form onSubmit={handleSearchSubmit} className="relative w-full">
                                  ...
                                  <Input
                                    ...
                                    className="h-20 rounded-[1.5rem] border-primary/30 pl-14 bg-white shadow-2xl focus:ring-primary text-lg"
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                  />
                                  <Button 
                                    type="button"
                                    variant="ghost" 
                                    className="absolute right-5 top-1/2 -translate-y-1/2 h-10 w-10 p-0 rounded-full"
                                ...
                                {(matchedCategories.length > 0 || suggestions.length > 0) && (
                                  <div className="bg-white rounded-[2rem] border border-border/50 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                                    {matchedCategories.length > 0 && (
                                      <div className="p-6 border-b bg-primary/5">
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-primary mb-4">Live Categories</p>
                                        <div className="flex flex-wrap gap-2.5">
                                          {matchedCategories.map((cat: any) => (
                                            <Link 
                                              ...
                                              href={`/shop?q=${cat.name}`}
                                              onClick={() => { setIsMobileSearchActive(false); setSearchQuery(''); }}
                                              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-primary/20 rounded-full text-xs font-bold text-secondary hover:bg-primary hover:text-white transition-all shadow-sm"
                                            >
                                        ...
                                        <div className="divide-y divide-border/30">
                                          {suggestions.map((product) => (
                                            <Link 
                                              ...
                                              onClick={() => { setIsMobileSearchActive(false); setSearchQuery(''); }}
                                            >
                                              <div className="relative w-14 h-14 rounded-2xl overflow-hidden shadow-md shrink-0 border border-black/5">
                                                <Image src={product.imageUrl} alt={product.title} fill className="object-cover" />
                                              </div>
                                              <div className="flex-1 min-w-0">
                                    ...
                                    <Button 
                                      variant="ghost" 
                                      className="w-full h-16 rounded-none text-primary font-bold text-sm gap-3 hover:bg-primary/5 border-t"
                                      onClick={() => handleSearchSubmit()}
                                    >
                              ...
                              <div className="grid grid-cols-2 gap-4 animate-in fade-in duration-500">
                                  <Button 
                                    variant="outline" 
                                    className="h-20 rounded-[1.5rem] border-muted-foreground/10 gap-4 justify-start px-8 group hover:border-primary/40 hover:bg-primary/5 transition-all shadow-sm"
                                    onClick={() => setIsMobileSearchActive(true)}
                                  >
                                      <Search className="h-6 w-6 text-primary group-hover:scale-110 transition-transform" />
                                      <span className="font-bold text-base">Find Pieces</span>
                                  </Button>
                                  <Button variant="outline" className="h-20 rounded-[1.5rem] border-muted-foreground/10 gap-4 justify-start px-8 relative group hover:border-primary/40 hover:bg-primary/5 transition-all shadow-sm" asChild>
                                      <Link href="/cart">
                                          ...
                                          {cartCount > 0 && (
                                            <span className="absolute top-1/2 -translate-y-1/2 right-8 w-7 h-7 bg-primary text-white text-xs flex items-center justify-center rounded-full font-bold shadow-lg ring-2 ring-white">
                                              {cartCount}
                                            </span>
                                          )}
                                      </Link>
                      ...
                      <div className="space-y-6">
                          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Discover</p>
                          <div className="grid gap-3">
                              {navLinks.map((link) => (
                                  <Link key={link.href} href={link.href} className={cn("flex items-center justify-between p-6 rounded-[1.5rem] transition-all duration-300", pathname === link.href ? "bg-primary text-white shadow-xl scale-[1.02]" : "bg-muted/40 text-secondary hover:bg-muted/60")}>
                                      <div className="flex items-center gap-5"><link.icon className="h-6 w-6" /><span className="text-xl font-bold">{link.label}</span></div>
                                      <ChevronRight className={cn("h-6 w-6 transition-transform", pathname === link.href ? "opacity-100 translate-x-1" : "opacity-30")} />
                                  </Link>
                              ))}
                          </div>
                      </div>
                  </div>
                  <div className="px-10 pt-10 pb-14 border-t bg-muted/30">
                    {user ? (
                      <div className="flex flex-col gap-4">
                        <Link href="/account" className="w-full">
                          <Button className="w-full h-16 rounded-[1.5rem] bg-secondary text-secondary-foreground text-lg font-bold gap-4 shadow-xl hover:scale-[1.02] transition-transform"><User className="h-6 w-6" />Account Settings</Button>
                        </Link>
                        <Button variant="outline" className="w-full h-14 rounded-2xl text-destructive font-bold border-destructive/10 hover:bg-destructive/5" onClick={handleSignOut}>Log Out</Button>
                      </div>
                    ) : (
                      <Link href={loginUrl}>
                        <Button className="w-full h-16 rounded-[1.5rem] bg-secondary text-secondary-foreground text-xl font-bold shadow-2xl animate-pulse-glow">Secure Sign In</Button>
                      </Link>
                    )}
                  </div>
                </SheetContent>
